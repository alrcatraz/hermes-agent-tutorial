...
## 4.4 使用 Hermes Desktop

Hermes Desktop 是官方提供的图形界面客户端，支持 **本地模式** 和 **远程连接** 两种使用方式：
...
首次启动会自动打开浏览器进入 Dashboard（默认 `http://localhost:8080`），
可以在图形界面中管理会话、配置 Provider、查看日志。

### 连接远程后端

如果 Hermes Agent 运行在另一台机器上（如服务器、VPS），可以本地启动 Desktop 并指向远程后端：
...
远程模式下的使用体验与本地完全一致——所有会话历史、工具调用、文件操作都在远端执行，
Desktop 只负责渲染界面。

!!!info "注意"
    连接远程后端时，它**无法直接使用Computer Use工具控制你当前的设备**。
