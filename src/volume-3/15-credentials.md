...
|:---------|:---------|:---------|:------------|:---------|
| **Web 登录凭据** | GitHub 密码、API Key、网站账号 | **KeePassXC** | `Combined.kdbx` | `keepassxc-cli` |
| **设备登录凭据** | SSH 密码、sudo 密码、服务器信息 | **GPG-encrypted YAML** | `~/Documents/credentials/` | `gpg` + `.env` 口令 |
...
打开 `Combined.kdbx` 数据库文件，通过 KeePassXC 图形界面浏览、搜索、复制凭据。

### 命令行查询（无头环境）

Hermes Agent 在无头环境或自动化脚本中通过 `keepassxc-cli` 查询凭据：

```bash
# 列出所有条目
keepassxc-cli ls ~/credentials/Combined.kdbx

# 搜索 GitHub 相关凭据
keepassxc-cli search ~/credentials/Combined.kdbx github

# 显示凭据详情（配合管道使用）
keepassxc-cli show ~/credentials/Combined.kdbx -s "github"
```
...
## 15.4 系统二：GPG-encrypted YAML — 设备登录凭据
...
### 实际部署：~/Documents/credentials/ 目录结构

Astra 生态的实际部署使用了以下布局，按凭据用途分类加密：

```
~/Documents/credentials/
├── README.md                              ← 索引（权限 600，仅所有者可读）
├── personal-credentials.yaml.gpg          ← 个人设备
├── work-credentials.yaml.gpg              ← 工作设备
├── other-credentials.yaml.gpg             ← 其他人（待补充）
└── infra-credentials.yaml.gpg.deprecated  ← 已废弃
```
...
| `other-credentials.yaml.gpg` | 其他人 | 朋友/家人设备（空待补充） |
...
echo "$PASS" | gpg --batch --no-tty --passphrase-fd 0 \
  ...
  --decrypt ~/Documents/credentials/device-credentials.yaml.gpg 2>/dev/null
```
