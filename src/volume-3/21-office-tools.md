# 第21章：Office 工具与文档撰写 {#ch:21}

## 21.1 OfficeCLI：无头 Office 文档处理
...
!!! info "参考说明"
    OfficeCLI 的完整 MCP 集成模式、视图模式、高级功能（Word/PPT/Excel 创建与编辑）在 [§21.4](#sec:21.4) 中有详细说明。本节仅介绍安装与基本工具链。

## 21.2 Pandoc：格式转换瑞士军刀
...
## 21.4 OfficeCLI：单二进制全功能 Office 文档处理 {#sec:21.4}
...
officecli add tutorial.docx /body paragraph text="第1章：入门" bold=true size=19
...
### 21.6.3 Root vs 非 Root：如何选择

上文的安装方案采用了**非 root** 安装，这也是推荐的默认选择：

| 场景 | 推荐 | 原因 |
|:-----|:-----|:------|
| **个人桌面**（最常见） | 非 root | 无需提权安装，不影响系统包管理器；更新 TeX Live 只需解压新 ISO 重新安装 |
| **多用户服务器** | root（`/usr/local/texlive/`） | 所有用户共享，无需逐个配置 `~/.local/bin` |
| **CI/CD 流水线** | 非 root 或 Docker 容器 | CI 环境通常无 root 权限，texlive 作为项目依赖安装到 workspace |

**非 root 优势总结：**
- 安装和卸载不影响系统包管理器（`zypper`/`apt`）
- 版本切换灵活（可保留多个 release 副本）
- 无需 `sudo` 执行 TeX 命令（二进制在 `~/.local/bin/`）

**何时选 root：**
- 多用户共享环境，每个用户都需要完整的 TeX 环境
- 服务器场景，作为系统基础设施提供
- 已经通过系统包管理器安装过 TeX Live 且工作良好

## 21.7 文档自动生成工作流

### 21.7.1 完整双输出管线（Web + PDF）
